...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    # constants.
    #meta_graph_def = tf.train.export_meta_graph(filename='/tf_files/my-model.meta')
    writer = tf.summary.FileWriter("/tf_files/", sess.graph)
    saver = tf.train.Saver()
    save_path = saver.save(sess,'/tf_files/my-model')

    output_graph_def = graph_util.convert_variables_to_constants(
        sess, graph.as_graph_def(), [FLAGS.final_tensor_name])
    #print("loss: "+str(tf.nn.l2_loss(output_graph_def)))
